...
<body>    
    ...
    <div class="container mt-3">
        <h1 class="display-4 text-center">Autores</h1>
                ...
                <div class="col-md-8">
                    ...
                    <!-- <form method="GET" action="{{ route('getAutores') }}"> -->
                        <div class="input-group mt-5 mb-5">
                            <input type="search" class="form-control rounded" placeholder="Search" aria-label="Search" aria-describedby="search-addon" 
                                name="autor_search" id="autor_search" value="{{ old('autor_search') }}"/>
                    ...
                    @if($autores->count() > 0)
                        ...
                        <tbody>
                            ...
                            <tr>
                                <th scope="row" style="width: 6%">{{ $autor->id }}</th>
                                <td style="width: 55%">{{ $autor->autor }}</td>                        
                                <td style="width: 1%">
                                    <form action="/autores/{{$autor->id}}/edit" method="get"><button class="btn btn-outline-warning"
                                            type="submit">Editar</button>
                                    </form>
                                </td>
                                <td style="width: 1%">
                                    <form action="/autores/{{$autor->id}}/edit" method="post">@method('DELETE')<button
                    ...
                    @else
                        {{Session::flash('message','No tienes `Autores` registrados!!!');}}
                        <div class="container">
    ...
    <script type="text/javascript">
        $('#autor_search').on('keyup',function(){
            $value=$(this).val();
            $.ajax({
                type : 'get',
                url : "{{URL::to('getAutores')}}",
                data: {'data_search':$value, 'table':'autores', 'field':'autor'},
