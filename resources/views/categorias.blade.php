...
<body>    
                ...
                <div class="col-md-8">
                    ...
                    <!-- <form method="GET" action="{{ route('getAutores') }}"> -->
                        <div class="input-group mt-5 mb-5">
                            <input type="search" class="form-control rounded" placeholder="Search" aria-label="Search" aria-describedby="search-addon" 
                                name="categoria_search" id="categoria_search" value="{{ old('categoria_search') }}"/>
                    ...
                    @if($categorias->count() > 0)
                        ...
                        <tbody>
                            ...
                            <tr>
                                <th scope="row" style="width: 6%">{{ $categoria->id }}</th>
                                <td style="width: 55%">{{ $categoria->categoria }}</td>                        
                                <td style="width: 1%">
                                    <form action="/categorias/{{$categoria->id}}/edit" method="get"><button class="btn btn-outline-warning"
                                            type="submit">Editar</button>
                                    </form>
                                </td>
                                <td style="width: 1%">
                                    <form action="/categorias/{{$categoria->id}}/edit" method="post">@method('DELETE')<button
                    ...
                    @else
                        {{Session::flash('message','No tienes `Categorías` registradas!!!');}}
                        <div class="container">
    ...
    <script type="text/javascript">
        $('#categoria_search').on('keyup',function(){
            $value=$(this).val();
            $.ajax({
                type : 'get',
                url : "{{URL::to('getCategorias')}}",
                data: {'data_search':$value, 'table':'categorias', 'field':'categoria'},
